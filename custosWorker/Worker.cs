...
namespace custosWorker
{
    ...
    public class Worker : BackgroundService
    {
       ...
       // private string apiUrl = "https://localhost:7237";
        ...
        private long networkThreshold = 314572800;  //give data in byte ( 24 MB) equal to 200Mbps
    ...
    public override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            _timer3 = new System.Threading.Timer(Custosdata, null, 0, 86400000);
        ...
        private async void Custosdata(object state)
        {
            try
            {
                Port port = new Port();
                ...
                osinfo.osinfo();
                OSCoreInformation oscoreinfo = new OSCoreInformation();
                oscoreinfo.oscoreinfo();
                HardDiskinfo hard = new HardDiskinfo();
                hard.diskdataInformation();
                AntivirusControl anti = new AntivirusControl();
                anti.GetAntiviruInfo();
                Deviceinfo deviceinfo = new Deviceinfo();
                deviceinfo.devicedata();
                Services ser = new Services();
                ser.servicedata();
                InstalledSoftware install = new InstalledSoftware();
                install.Installed();
                SoftwareInfo soft = new SoftwareInfo();
                soft.GetSoftwareInfo();
                ...
                await Task.Run(() =>
                {
                    Parallel.ForEach(Process.GetProcesses(), p =>
                    {
                        try
                        {
                            ...
                            long memorySize = p.WorkingSet64 /1024 / 1024;
        ...
        private double CpuUsages()
        {
            try
            {
                ...
                cpuCounter.NextValue(); // Initial call to "warm up" the counter
                Thread.Sleep(1000); // Wait a second to get a sample

                double cpuUsage = cpuCounter.NextValue(); // Get the actual CPU usage value
                double cpuUsagePerCore = cpuUsage / Environment.ProcessorCount; // If you need per core usage

                return Math.Round(cpuUsage); // Returning rounded per-core CPU usage
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error retrieving CPU usage: {ex.Message}");
                Console.WriteLine($"Error retrieving CPU usage: {ex}");
                return 0;
            }
        }
